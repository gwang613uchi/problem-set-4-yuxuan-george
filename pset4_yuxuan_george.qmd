...
3. 
```{python}
# Count the number of active hospitals by ZIP code and year
active_per_zip_code = pos_all_years[pos_all_years['PGM_TRMNTN_CD'] == 0].groupby(['ZIP_CD', 'Year']).size().reset_index(name='Active_Hospitals')

# Merge with closures_df to add active hospitals count
closures_df = closures_df.merge(
    active_per_zip_code[['ZIP_CD', 'Year', 'Active_Hospitals']].rename(columns={'Year': 'CLS_YR', 'Active_Hospitals': 'Active_Hospitals_closure'}),
    on=['ZIP_CD', 'CLS_YR'], 
    how='left'
)
```
...
```{python}
# Merge again to add active hospitals count in the year after
closures_df = closures_df.merge(
    active_per_zip_code[['ZIP_CD', 'Year', 'Active_Hospitals']].rename(columns={'Year': 'CLS_YR_next', 'Active_Hospitals': 'Active_Hospitals_next_year'}),
    left_on=['ZIP_CD', 'CLS_YR'], 
    right_on=['ZIP_CD', 'CLS_YR_next'], 
    how='left'
)

# Drop the 'CLS_YR_next' column
closures_df = closures_df.drop(columns=['CLS_YR_next'])

# Remove hospitals where active hospital count does not decrease in the next year
closures_df_filtered = closures_df[
    (closures_df['Active_Hospitals_closure'] > closures_df['Active_Hospitals_next_year']) 
    ]
```

    a.
```{python}
# Identify hospitals that fit the merger/acquisition definition
merger_acquisition_hospitals = closures_df[
    (closures_df['Active_Hospitals_closure'] <= closures_df['Active_Hospitals_next_year']) &
    (closures_df['Active_Hospitals_next_year'].notna())
]

# Number of hospitals that fit the definition
num_merger_acquisition_hospitals = merger_acquisition_hospitals.shape[0]

# Display 
num_merger_acquisition_hospitals
```
...
ax.set_title("Number of Hospitals per ZIP Code in Texas (2016) (Count)", fontsize=16)
ax.set_axis_off()
